...
"""
ZMQ Listener connector for uPKI CA server.

This module provides the ZMQListener class which extends the base Listener
class to handle certificate operations via ZeroMQ communication.
"""
...
class ZMQListener(Listener):
    """ZMQ-based CA Server listener.

    This class extends the base Listener class to handle certificate
    operations including registration, generation, signing, renewal,
    and revocation via ZeroMQ communication.
    ...
    def _upki_list_admins(self, params: dict) -> list:
    ...
    def _upki_add_admin(self, dn: str) -> bool:
        ...
        Raises:
            Exception: If DN is missing or operation fails.
        """
        if dn is None:
    ...
    def _upki_remove_admin(self, dn: str) -> bool:
    ...
    def _upki_list_profiles(self, params: dict) -> dict:
        ...
        Returns:
            Dictionary of profile names to configuration data.
        """
        return self._profiles.list()

    def _upki_profile(self, profile_name: str) -> dict:
        ...
        Raises:
            Exception: If profile name is missing or profile doesn't exist.
        """
        ...
        try:
            ...
            data = self._profiles.load(profile_name)
        except Exception as err:
            raise Exception(err)

        return data

    def _upki_add_profile(self, params: dict) -> bool:
        """Add a new profile.

        Args:
            params: Dictionary containing 'name' and profile data.

        Returns:
            True if profile added successfully.

        Raises:
            Exception: If profile name is missing or operation fails.
        """
        try:
            name = params["name"]
        except KeyError:
    ...
    def _upki_update_profile(self, params: dict) -> bool:
        """Update an existing profile.

        Args:
            params: Dictionary containing 'name', 'origName', and profile data.

        Returns:
            True if profile updated successfully.

        Raises:
            Exception: If required parameters are missing or operation fails.
        """
        try:
            name = params["name"]
        except KeyError:
    ...
    def _upki_remove_profile(self, params: dict) -> bool:
        """Remove a profile.

        Args:
            params: Dictionary containing 'name' of profile to remove.
    ...
    def _upki_get_options(self, params: dict) -> dict:
        """Get allowed options.
        ...
        Returns:
            Dictionary of allowed option values.
        """
        return vars(self._profiles._allowed)

    def _upki_list_nodes(self, params: dict) -> list:
        """List all nodes.
        ...
        Returns:
            List of node dictionaries with humanized serials.

        Raises:
            Exception: If listing nodes fails.
        """
        try:
    ...
    def _upki_get_node(self, params: dict) -> dict:
        ...
        Returns:
            Node dictionary with humanized serial.
    ...
    def _upki_download_node(self, dn: str) -> str:
        """Download a node's certificate.
        ...
        Raises:
            Exception: If node doesn't exist or certificate is not valid.
        """
        try:
    ...
    def _upki_register(self, params):
        ...
        try:
            self.output(
                "Register node {n} with profile {p}".format(n=cn, p=params["profile"])
            )
            res = self._storage.register_node(
                dn,
                params["profile"],
                profile,
                sans=clean["sans"],
    ...
    def _upki_generate(self, params):
        ...
        if not self._storage.exists(dn):
            ...
            try:
                ...
                raise Exception(
                    "Unable to register node dynamically: {e}".format(e=err)
                )
        ...
        try:
            ...
            self.output(
                "Generating private key based on {p} profile".format(p=profile_name)
            )
        ...
        try:
            ...
            raise Exception(
                "Unable to generate Certificate Signing Request: {e}".format(e=err)
            )
        ...
        try:
            ...
            raise Exception(
                "Unable to store Server Certificate Request: {e}".format(e=err)
            )

        try:
            self.output(
                "Activate node {n} with profile {p}".format(n=dn, p=profile_name)
            )
    ...
    def _upki_update(self, params):
        ...
        if not self._storage.exists(dn):
            raise Exception(
                "This node does not exists. Note: DN (and so CN) are immutable once registered."
            )
        ...
        try:
            self.output(
                "Update node {n} with profile {p}".format(n=cn, p=params["profile"])
            )
            res = self._storage.update_node(
                dn,
                params["profile"],
                profile,
                sans=clean["sans"],
    ...
    def _upki_sign(self, params):
        ...
        try:
            ...
            csr_file = self._storage.store_request(
                csr_pem, nodename="{p}.{c}".format(p=node["Profile"], c=cn)
            )
            crt_file = self._storage.store_public(
                crt_pem, nodename="{p}.{c}".format(p=node["Profile"], c=cn)
            )
        except Exception as err:
        ...
        return {
            ...
            "certificate": crt_pem.decode("utf-8"),
        }
    ...
    def _upki_renew(self, params):
        ...
        try:
            csr_pem = self._storage.download_request(
                "{p}.{n}".format(p=node["Profile"], n=cn)
            )
        except Exception as err:
        ...
        until_expire = (
            datetime.datetime.fromtimestamp(node["Expire"])
        ...
        try:
            self.output(
                "Renew node {n} with profile {p}".format(n=dn, p=node["Profile"])
            )
        ...
        try:
            ...
            self._storage.store_public(
                pub_pem, nodename="{p}.{c}".format(p=node["Profile"], c=node["CN"])
            )
        except Exception as err:
        ...
        return {
            ...
            "dn": dn,
            "profile": node["Profile"],
            "certificate": pub_pem.decode("utf-8"),
        }
    ...
    def _upki_ocsp_check(self, params):
        ...
        try:
            (status, rev_time, rev_reason) = self._storage.is_valid(
