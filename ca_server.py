...
This module provides the main entry point for the uPKI Certificate Authority server.
It handles initialization, registration, and listening modes of operation.
"""
...
def main(argv: list[str]) -> None:
    """Main entry point for the uPKI CA server.

    Handles command-line argument parsing and orchestrates the CA server
    initialization, registration, and listening modes.

    Args:
        argv: Command-line arguments list (typically sys.argv).
    """
    ...
    parser = argparse.ArgumentParser(
        description="µPki [maɪkroʊ ˈpiː-ˈkeɪ-ˈaɪ] is a small PKI in python that should let you make basic tasks without effort."
    )
    ...
    parser.add_argument(
        "-l",
        "--log",
        help="Define log file (default: {f})".format(f=LOG_PATH),
        default=LOG_PATH,
    )
    parser.add_argument(
        "-p",
        "--path",
        help="Define uPKI directory base path for config and logs (default: {p})".format(
            p=BASE_DIR
        ),
        default=BASE_DIR,
    )
    ...
    parser_init.add_argument(
        "-c",
        "--ca",
        help="Import CA keychain rather than generating. A path containing 'ca.key, ca.csr, ca.crt' all in PEM format must be defined.",
    )

    parser_register = subparsers.add_parser(
        "register",
        help="Enable the 0MQ server in clear-mode. This allow to setup your RA certificates.",
    )
    parser_register.set_defaults(which="register")
    parser_register.add_argument(
        "-i", "--ip", help="Define listening IP", default=LISTEN_HOST
    )
    parser_register.add_argument(
        "-p", "--port", help="Define listening port", default=LISTEN_PORT
    )

    parser_listen = subparsers.add_parser(
        "listen",
        help="Enable the 0MQ server in TLS. This enable interactions by events emitted from RA.",
    )
    ...
    try:
        logger = upkica.core.UpkiLogger(
            LOG_PATH, level=LOG_LEVEL, proc_name="upki_ca", verbose=VERBOSE
        )
